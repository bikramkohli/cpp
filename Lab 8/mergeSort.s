...
mergeSort:
	...
	jge end 		;jump to end
	xor r12, r12	;clearing r12 for mid variable
	add r12, rsi 	;adding rsi (start) to r12
	add r12, rdx 	;adding rdx (end) to r12
	shr r12, 1 		;dividing r12 by 2 (r12 -> (start + end)/2 -> mid)
	push rdx 		;storing end in stack
	mov rdx, r12 	;setting end to mid
	call mergeSort	;mergeSort(rdi(arr), rsi(start), rdx(mid))
	pop rdx 		;popping rdx(end)
	inc r12 		;mid += 1
	push rsi 		;storing start in stack
	mov rsi, r12 	;setting start to mid + 1
	call mergeSort 	;mergeSort(rdi(arr), rsi(mid + 1), rdx(end))
	pop rsi 		;popping rsi(start)
	xor rcx, rcx 	;zeroing 4th parameter for merge
	mov rcx, rdx 	;for merge, right has to be 4th parameter
	dec r12			;mid -= 1 (returns to original mid)
	push rdx 		;storing end in stack
	mov rdx, r12	;for merge, mid has to be 3rd parameter
	call merge 		;merge(rdi(arr), rsi(start), rdx(mid), rcx(end))
	pop rdx 		;popping rdx(end)
